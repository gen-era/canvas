...
from canvas.models import Chip, ChipType, Sample, SampleType, Institution, SampleChip, Lot
        ...
        for index, row in df.iterrows():
            try:
                ...
                if pd.isna(concentration):
                    self.stderr.write(self.style.ERROR(f'NaN value for concentration at row {index + 1}. Skipping row.'))
                    continue
                ...
                if pd.isna(call_rate):
                    self.stderr.write(self.style.ERROR(f'NaN value for call rate at row {index + 1}. Skipping row.'))
                    continue
                ...
                try:
                    ...
                    self.stderr.write(self.style.WARNING(f'Invalid arrival date at row {index + 1}. Replacing with default date.'))

                chip_type, _ = ChipType.objects.get_or_create(name=row['Çip'])
                institution, _ = Institution.objects.get_or_create(name=row['Kurum'])
                sample_type, _ = SampleType.objects.get_or_create(name='Default')  # Adjust as needed
                ...
                sample_chip, _ = SampleChip.objects.get_or_create(
                    sample=sample,
                    chip=chip,
                ...
                self.stdout.write(self.style.SUCCESS(f'Successfully processed row {index + 1}'))

            except ValidationError as ve:
                self.stderr.write(self.style.ERROR(f'Validation error: {ve} at row {index + 1}'))
            except Exception as e:
